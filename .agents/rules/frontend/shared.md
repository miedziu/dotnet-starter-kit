...
Applies to  `clients/dashboard`. Read this for any React work, then read
the app-specific file `dashboard.md` for divergences.
...
Login `POST /api/v1/identity/token/issue` with header `X-FSH-App: "dashboard"`. **localStorage keys are namespaced per app** `fsh.dashboard.*`. Permission *source* differs per app — see the app files.
...
Then apply the app-specific steps in `dashboard.md` (forms, permission gating, suspense, etc.).
