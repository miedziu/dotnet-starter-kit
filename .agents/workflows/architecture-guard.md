---
description: Verify changes don't violate architectural integrity — module boundaries, Lib protection, the four-place module registration, and the architecture suite. Run before commit/PR. READ-ONLY.
---
...
### 1. Lib guard
```bash
git diff --name-only | grep -E "^s/Lib/"
```
Any hit → **STOP and flag**: Lib changes need explicit approval (wide blast radius).

### 2. Build clean
```bash
dotnet build s/FSH.Starter.slnx 2>&1 | grep -E "warning|error"   # expect none (TreatWarningsAsErrors)
```

### 3. Module boundary heuristic
```bash
grep -rn "using FSH.Mod\." s/Mod --include="*.cs" | grep -v "\.Contracts"
```
...
```bash
grep -rn "{New}Module\|{New}ContractsMarker" s/Host/FSH.Starter.Api/Program.cs s/Host/FSH.Starter.DbMigrator/Program.cs
```
...
```bash
grep -rn "IRequiredPermissionMetadata" s --include="*.cs"
```
...
Lib      : ✅ untouched | ⚠️ MODIFIED — needs approval
Build               : ✅ 0 warnings | ❌ {n}
