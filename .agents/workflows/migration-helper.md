...
- All migrations live in **one** project, `src/Host/FSH.Starter.Migrations.PostgreSQL`, foldered **per module/context** (`Chat/`, `Identity/`, …), each with its own `{X}DbContextModelSnapshot`.
...
## Context names (real)
`IdentityDbContext`, `AuditDbContext`, `BillingDbContext`, `TicketsDbContext`, `FilesDbContext`, `ChatDbContext`, `NotificationsDbContext`, `WebhookDbContext`.
