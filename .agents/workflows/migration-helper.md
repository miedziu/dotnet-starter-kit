...
- All migrations live in **one** project, `s/Host/FSH.Starter.Migrations.PostgreSQL`, foldered **per module/context** (`Chat/`, `Identity/`, …), each with its own `{X}DbContextModelSnapshot`.
- Startup project is `s/Host/FSH.Starter.Api`. Always pass `--context {X}DbContext` and `--output-dir {X}`.
...
## Context names (real)
`IdentityDbContext`, `AuditDbContext`, `BillingDbContext`, `TicketDbContext`, `FileDbContext`, `ChatDbContext`, `NotificationsDbContext`, `WebhookDbContext`.

## Apply (canonical path)
```bash
dotnet run --project s/Host/FSH.Starter.DbMigrator -- list-pending   # preview
...
| "Build failed" | `dotnet build s/FSH.Starter.slnx` first |
