...
Identity, billing, audit, webhooks, file, chat, real-time, caching, jobs, storage, OpenAPI and OpenTelemetry — already wired, and **100% yours as source** (no black-box packages).
...
### [📖 Documentation](https://fullstackhero.net) · [🚀 Get Started](https://fullstackhero.net/docs/getting-started/introduction/) · [🧩 Mod](https://fullstackhero.net/docs/modules/) · [🏗️ Architecture](https://fullstackhero.net/docs/architecture/) · [📦 Changelog](https://fullstackhero.net/docs/changelog/)
...
fsh new MyApp
cd MyApp
dotnet run --project s/Host/FSH.Starter.AppHost   # 🎉 whole stack up: API + React app + Postgres + Redis + MinIO
```
...
- **Observability**: Serilog structured logging + **OpenTelemetry** traces/metrics/logs, health probes, security/exception audit.
...
### Mod (bounded contexts)
**Identity · Billing · Ticket · Chat · File · Webhooks · Audit · Notifications** — each a runtime project plus a `.Contracts` project (its only public surface).
...
| Path | What |
|---|---|
| `s/Lib/` | Shared framework libraries (Core, Persistence, Web, Caching, Eventing, Eventing.Abstractions, Jobs, Mailing, Storage) |
| `s/Mod/{Name}/` | Bounded contexts — each with a runtime project + a `.Contracts` project (its public API) |
| `s/Host/FSH.Starter.Api` | Composition-root Web API host |
| `s/Host/FSH.Starter.AppHost` | .NET Aspire orchestrator (Postgres, Redis, MinIO, migrator, API, React app) |
| `s/Host/FSH.Starter.DbMigrator` | One-shot migrate/seed runner (DB is **not** migrated at API startup) |
| `c` | The React app |
...
- [Mod](https://fullstackhero.net/docs/modules/) — Identity, Ticket, Chat, and more
