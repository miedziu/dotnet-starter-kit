...
public sealed class InvoiceConfiguration : IEntityTypeConfiguration<Invoice>
{
    public void Configure(EntityTypeBuilder<Invoice> builder)
    {
        ...
        builder.Property(x => x.Purpose).HasConversion<int>().HasDefaultValue(Spec.InvoicePurpose.Usage);
        builder.Property(x => x.PeriodStartUtc);
        builder.Property(x => x.PeriodEndUtc);
        builder.Property(x => x.Notes).HasMaxLength(2048);

        // One invoice per month *per purpose* — subscription (term base fee) and usage
        // (metered overage) are separate streams that may both fall in the same calendar month.
        // Recurring invoices are unique per period/purpose; Topup invoices (Purpose=2) are
        // ad-hoc and may repeat within a period, so exclude them from the uniqueness filter.
        builder.HasIndex(x => new { x.PeriodYear, x.PeriodMonth, x.Purpose })
            .IsUnique()
            .HasFilter($"\"Purpose\" <> {(int)Spec.InvoicePurpose.Topup}")
            .HasDatabaseName("ux_invoices_period_purpose");
